...
import { Database, Monitor, Smartphone, Cpu, ChevronRight, Globe, Zap } from 'lucide-react';
import Button from '../atoms/Button';
import { useProjects } from '../../hooks/useProjects';

/**
 * Componente ScrollStory: El motor de Scrollytelling 3D.
 * Gestiona la narrativa visual inmersiva mediante el mapeo del scroll a transformaciones 3D.
 */
const ScrollStory: React.FC = () => {
  // useRef: Referencia al contenedor que define la longitud del scroll (1200vh).
  const containerRef = useRef<HTMLDivElement>(null);
  const navigate = useNavigate();
  const { projects } = useProjects();

  // Filtramos proyectos que están en fase de laboratorio para la última escena.
  const inProgressProjects = projects.filter(p => p.status === 'in_progress');

  // useScroll: Captura el progreso del scroll (0 a 1) dentro del contenedor objetivo.
  const { scrollYProgress } = useScroll({
    target: containerRef,
    offset: ["start start", "end end"]
  });

  // useSpring: Aplica físicas de resorte para suavizar el movimiento y dar una sensación premium.
  ...
  // --- MATEMÁTICAS DE TRANSFORMACIÓN (MAPPING) ---

  // ESCENA 1: EL NOMBRE (Zoom Infinito)
  ...
  // ILUMINACIÓN DINÁMICA: Opacidad de las auras de fondo según el progreso.
  ...
  // FONDO DE GRADIENTE RADIAL MOVIBLE
  ...
  return (
    <div ref={containerRef} className="relative h-[1200vh] bg-space-black font-sans overflow-clip">
      ...
      {/* CAPA DE LUCES (AURAS) - Sincronizada con el scroll */}
      <motion.div style={{ background: bgGlow }} className="fixed inset-0 pointer-events-none z-0" />

      <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden">
         <motion.div style={{ opacity: purpleGlowOpacity }} className="absolute -top-[10%] -right-[10%] w-[70%] h-[70%] bg-cyber-purple/20 blur-[120px] rounded-full" />
         <motion.div style={{ opacity: cyanGlowOpacity }} className="absolute -bottom-[10%] -left-[10%] w-[60%] h-[60%] bg-cyber-cyan/15 blur-[120px] rounded-full" />
      </div>

      <div className="sticky top-0 h-screen w-full flex items-center justify-center overflow-hidden">

        {/* ESCENA 1: EL NOMBRE (Intro) */}
        <motion.div
        ...
        >
          <motion.span initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1.2, delay: 0.5 }} className="text-cyber-purple font-mono text-[9px] md:text-[10px] tracking-[0.8em] mb-8 uppercase">
            ChopCode Solutions presents
          </motion.span>
          <div className="flex flex-col items-center">
            <motion.h1 initial={{ opacity: 0, filter: "blur(20px)" }} animate={{ opacity: 1, filter: "blur(0px)" }} transition={{ duration: 1.5, delay: 0.8 }} className="text-5xl sm:text-7xl md:text-[11rem] font-black tracking-tighter leading-[0.75] text-white select-none text-center">
        ...
        >
          <div className="bg-[#0A0A0A] border border-white/10 rounded-[3rem] p-10 md:p-20 text-center backdrop-blur-md w-full relative overflow-hidden shadow-2xl">
            ...
            <h2 className="text-3xl md:text-6xl font-black mb-12 tracking-tighter uppercase italic text-white leading-none text-gradient">Construyendo el futuro.</h2>
            ...
            <Button onClick={() => navigate('/ecosystem')} variant="primary" className="!px-16 !py-6 !text-[11px] uppercase tracking-[0.4em] font-black">
